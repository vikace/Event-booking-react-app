...
export default function BookingsCard({data,removeEventListner}) {
  ...
  {
    e.preventDefault();
    axios.delete(getPathname(data._links.booking.href)).then(response=>{
      setRemoveStatus(response.status);
      console.log(data._links.booking.href);
      removeEventListner(data._links.booking.href);
